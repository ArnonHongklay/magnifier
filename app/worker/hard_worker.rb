class HardWorker
  ...
  def cpu(server)
    ...
    url = "http://#{server.ip_address}:1337/cpu_info"
  ...
  def memory(server)
    ...
    url = "http://#{server.ip_address}:1337/memory_info"
    response = Net::HTTP.get_response(URI.parse(url))
    items = JSON.parse(response.body)
  ...
  def load_avg(server)
    ...
    url = "http://#{server.ip_address}:1337/load_avg"
    response = Net::HTTP.get_response(URI.parse(url))
    items = JSON.parse(response.body)
  ...
  def bandwidth(server)
    url = "http://#{server.ip_address}:1337/bandwidth"
    response = Net::HTTP.get_response(URI.parse(url))
    items = JSON.parse(response.body)
  ...
  def io(server)
    url = "http://#{server.ip_address}:1337/io_stats"
    response = Net::HTTP.get_response(URI.parse(url))
    items = JSON.parse(response.body)
