class Bloggy::Post < ActiveRecord::Base
  ...
  scope :default,  -> {order("published_at DESC, bloggy_posts.created_at DESC, bloggy_posts.updated_at DESC") }
  ...
  def tag!(tags_attr)
    self.tags = tags_attr.map(&:strip).reject(&:blank?).map do |tag|
      Bloggy::Tag.where(name: tag).first_or_create
    end
  end

  def full_url
    "#{Bloggy::Engine.routes.url_helpers.root_path}#{self.url}"
  end
  ...
  def self.page p
    paged_results(p, Bloggy::Config.posts_per_page || 10, false)
  end

  def self.listing_page(p)
    paged_results(p, Bloggy::Config.admin_posts_per_page || 50, true)
  end
  ...
  def url_do_not_start_with_slash
    errors.add(:url, I18n.t("activerecord.errors.models.bloggy/post.attributes.url.start_with_slash")) if self.url.start_with?("/")
  end
end
